...
def main():
	time=str(health.datetime.now())[:19]
	...
	health.printer("Height:"+str(int(height/12))+"ft "+str(int(height%12))+"in | Weight:"+str(weight)+"lbs")
	
	
	bmi=health.bmi_func(weight,height)
	bmr=health.basal_metabolic_rate(sex,age,weight,height)
	cal=health.calorie_needs(sex,bmr)
	...
	if result!="n":
		...
		health.printer("Chest:"+str(chest)+"in| Waist:"+str(waist)+"in| Hip:"+str(hip)+"in| Forearm:"+str(forearm)+"in| Wrist:"+str(wrist)+"in\n")
		lbm,bf=health.body_fat(weight,sex,wrist,waist,hip,forearm)
		bai=health.body_adipose_index(height,hip,age,sex)
		wht=health.waist_to_height_ratio(waist, height, sex)
		whr=health.waist_to_hip_ratio(waist, hip, sex)
		if sex=="male":
			pig=health.pignet_index(chest,weight,height)
		else :  pig=''
	else:
		wrist=''
		waist=''
	...
	if rxr!='':
		health.maintain_record(name,time,age,sex,height,weight,bmi,bmr,cal,wrist,forearm,chest,waist,hip,lbm,bf,bai,wht,whr,pig,rxr)
		history=raw_input("Do you want to check your history of measurements? (Doesn't apply to new users) > ")
		if history!="n" :
			health.past_records(rxr)
		health.send_mail(rxr)
